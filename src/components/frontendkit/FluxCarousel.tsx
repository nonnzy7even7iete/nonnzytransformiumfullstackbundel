 ...
 * Isolé pour maintenance chirurgicale.
 */
...
export default function MiningDashboard() {
  ...
  const handleDragEnd = (_: any, info: any) => {
    const threshold = 30;
    // .info.offset.x : Accès point-notation pour capter le vecteur de swipe
  ...
  return (
    <div className="relative h-screen w-full bg-[var(--background)] overflow-hidden flex items-center justify-center font-sans">
      ...
      {/* ZONE DE CAPTURE DRAG (CENTRALISÉE POUR LA NAVBAR) 
          h-[65vh] : On laisse de l'espace en haut pour NavbarFront
          top-[15vh] : Centrage vertical du gesture
      */}
      <motion.div
        drag="x"
...
function Card({ item, position }: { item: any; position: number }) {
  ...
  // .window.innerWidth : Calcul dynamique du décalage pour l'effet galerie
  const xOffset = typeof window !== "undefined" && window.innerWidth < 768 ? window.innerWidth * 0.45 : 480;

  useEffect(() => {
    if (isActive && counterRef.current) {
      // .animate() : Animation numérique haute précision (notation point)
      const controls = animate(0, item.value, {
        duration: 2.2,
        ease: [0.16, 1, 0.3, 1],
        onUpdate: (v) => {
          // .current.textContent : Injection directe DOM pour performance 60fps
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "backdrop-blur-[35px] border border-[var(--foreground)]/10",
        isActive
          ? "shadow-[0_40px_100px_rgba(0,0,0,0.7),inset_0_0_20px_rgba(16,185,129,0.02)] bg-[var(--foreground)]/[0.03] border-emerald-500/20"
          : "bg-transparent border-none"
      )}
      style={{ borderRadius: "var(--radius-vercel, 12px)" }}
    >
      {/* INDICATEUR LED SOUVERAIN */}
      <div className={cn(
        "absolute top-6 right-6 h-1.5 w-1.5 rounded-full transition-all duration-1000",
        isActive ? "bg-emerald-500 shadow-[0_0_12px_#10b981]" : "bg-white/5"
      )} />

      {/* HEADER : HIERARCHIE CLAIRE */}
      <div className="relative z-10 flex flex-col gap-1.5">
        <p className="text-[9px] font-black tracking-[0.4em] uppercase text-emerald-500/80">
          {item.status}
        </p>
        <h3 className="text-[13px] font-bold text-[var(--foreground)] tracking-[0.1em] uppercase leading-tight">
          {item.title}
        </h3>
      </div>

      {/* BODY : DÉTAILS AUDIT (Lisibilité augmentée) */}
      <div className="relative z-10 mt-5 flex-1">
        <div className="flex flex-col gap-3 border-l border-white/10 pl-4 py-1">
          {item.notes.map((note: string, idx: number) => (
            <p key={idx} className="text-[10px] font-mono text-[var(--foreground)] opacity-50 italic leading-none tracking-tight">
              {note}
            </p>
          ))}
          <p className="text-[10px] font-medium text-[var(--foreground)] opacity-30 uppercase leading-relaxed max-w-[220px]">
            {item.detail}
          </p>
        </div>
      </div>
      ...
      <div className="relative z-10 flex items-baseline gap-3 mt-6">
        <h2 
          ref={counterRef} 
          className="text-6xl md:text-7xl font-black italic text-[var(--foreground)] tracking-tighter leading-none"
        >
          0.0
        </h2>
        <span className="text-xl font-bold opacity-20 italic text-emerald-500 uppercase tracking-tighter">
          {item.unit}
        </span>
      </div>

      {/* FOOTER : MÉTA-DONNÉES */}
      <div className="relative z-10 flex justify-between items-end pt-6 border-t border-white/5 mt-5">
        <div className="flex flex-col">
          <span className="text-[7px] font-mono opacity-20 uppercase tracking-widest text-white">Node_Reference</span>
          <span className="text-[9px] font-mono opacity-40 text-[var(--foreground)] uppercase">{item.ref}</span>
        </div>
        <span className="text-[10px] font-black italic text-emerald-500/90">{item.progression}</span>
      </div>
    </motion.div>
  );
}